...
# ============================================================
# Satellite Only Model - Complete Pipeline
# ============================================================
# 
# This script runs the complete pipeline for Satellite Only:
# 1. Optuna hyperparameter search (75 trials)
# 2. Retrain with best parameters (200 epochs)
# 3. Generate predictions
#
# Usage:
#   chmod +x solar_forecast/run_satellite_pipeline.sh
#   ./solar_forecast/run_satellite_pipeline.sh [optuna|retrain|predict|all]
#   ./solar_forecast/run_satellite_pipeline.sh all                  
# ============================================================
...
run_optuna() {
    ...
    echo "  Estimated time: 6-8 hours (CPU)"
    echo ""
...
run_retrain() {
    ...
    if [ ! -f "$OUTPUT_DIR/best_params.yaml" ]; then
        ...
        echo "        Run: ./run_satellite_pipeline.sh optuna"
        exit 1
    fi
    ...
    python -m solar_forecast.retrain_satellite_best \
...
run_predict() {
    ...
    if [ ! -f "$OUTPUT_DIR/best_model.pt" ]; then
        ...
        echo "        Run: ./run_satellite_pipeline.sh retrain"
        exit 1
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "     Time: ${MINUTES} minutes"
        echo "     Output: data/processed/predictions/satellite_only_predictions.csv"
        echo ""
    else
...
run_all() {
    ...
    echo "  1. Optuna Search (~6-8h CPU)"
    echo "  2. Retrain (~20-40min)"
    echo "  3. Predictions (~5-10min)"
    echo ""
    echo "Total estimated time: 7-9 hours (CPU)"
    echo ""
    ...
    echo "  - Predictions:    data/processed/predictions/satellite_only_predictions.csv"
...
show_usage() {
    echo ""
    echo "Usage: ./run_satellite_pipeline.sh [command]"
    echo ""
    echo "Commands:"
...
main() {
    echo ""
    echo "================================================================"
    echo "  Satellite Only Model Pipeline"
    echo "================================================================"
    echo ""
