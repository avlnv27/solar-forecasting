...
def load_fusion_model(
    ...
    logger.success(f"Loaded fusion model from {ckpt_path}")
    return model
...
def run_predict(
    ...
    logger.success(f"Prediction windows: {len(valid_starts)}")

    # ---- model
    edge_index = edge_index.to(device)
    edge_weight = edge_weight.to(device)

    model = load_fusion_model(cfg, edge_index, edge_weight, ckpt_path, device)
    ...
    csv_path = out_dir / "fusion_predictions.csv"
    df.to_csv(csv_path, index=False)

    logger.success(f"Predictions saved to {csv_path}")
