"""
Retrain Ground Only model with best hyperparameters from Optuna.

Run:
python -m solar_forecast.retrain_ground_best --epochs 200
"""
...
app = typer.Typer(help="Retrain Ground Only with best Optuna hyperparameters")
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, edge_index, edge_weight, max_grad_norm):
    model.train()
    total_loss, n = 0.0, 0
...
@torch.no_grad()
def evaluate(model, dataloader, criterion, device, edge_index, edge_weight):
    model.eval()
    total_loss, n = 0.0, 0
...
def retrain(
    ...
    """Retrain Ground Only with best Optuna params."""
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"Using device: {device}")
    
    # Load best params
    optuna_dir = MODELS_DIR / "best_optuna_ground"
    params_path = optuna_dir / "best_params.yaml"
    
    if not params_path.exists():
        logger.error(f"❌ Best params not found at {params_path}")
        logger.error("   Run: python -m solar_forecast.optuna_ground_only")
        raise typer.Exit(1)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Ground Only Training")
    plt.legend()
    plt.grid(alpha=0.3)
