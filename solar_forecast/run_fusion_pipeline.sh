...
# ============================================================
# Fusion Model - Complete Pipeline
# ============================================================
# 
# This script runs the complete pipeline for Fusion Model:
# 1. Optuna hyperparameter search (100 trials)
...
run_optuna() {
    ...
    echo "  Estimated time: 10-12 hours (CPU)"
    echo ""
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f "$OUTPUT_DIR/best_params.yaml" ]; then
            ...
            cat "$OUTPUT_DIR/best_params.yaml" | head -15
            echo ""
        fi
    else
...
run_retrain() {
    ...
    if [ ! -f "$OUTPUT_DIR/best_params.yaml" ]; then
        ...
        echo "        Run: ./run_fusion_pipeline.sh optuna"
        exit 1
    fi
    ...
    python -m solar_forecast.retrain_fusion_best \
...
run_predict() {
    ...
    if [ ! -f "$OUTPUT_DIR/best_model.pt" ]; then
        ...
        echo "        Run: ./run_fusion_pipeline.sh retrain"
        exit 1
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "     Time: ${MINUTES} minutes"
        echo "     Output: data/processed/predictions/fusion_predictions.csv"
        echo ""
    else
...
run_all() {
    ...
    echo "  1. Optuna Search (~10-12h CPU)"
    echo "  2. Retrain (~30-60min)"
    echo "  3. Predictions (~5-10min)"
    echo ""
    echo "Total estimated time: 11-13 hours (CPU)"
    echo ""
    ...
    echo "  - Predictions:    data/processed/predictions/fusion_predictions.csv"
...
show_usage() {
    echo ""
    echo "Usage: ./run_fusion_pipeline.sh [command]"
    echo ""
    echo "Commands:"
